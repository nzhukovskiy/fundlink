...
export class ChatsController {
    ...
    createChat(@Body() createChatDto: CreateChatDto, @Req() req) {
        if (req.token.payload.role === Roles.STARTUP) {
            ...
            return this.chatsService.createChat(req.token.payload.id, createChatDto.investorId);
        }
            ...
            return this.chatsService.createChat(createChatDto.startupId, req.token.payload.id);
        }
    }
}
